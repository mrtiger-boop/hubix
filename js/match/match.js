const Match = {
  ...
  init() {
    ...
    DOM.el("privateInput")?.addEventListener("keydown", e => { if (e.key === "Enter") this.send(); });
    this.emojis();
  },
  ...
  active(id) { return document.querySelector(`#${id} .active`)?.dataset.value || "Peu importe"; },
  ...
  async search() {
    ...
    const timer = setInterval(() => {
      ...
      if (index > steps.length) { clearInterval(timer); this.find(); }
    }, 550);
  },
  ...
  async find() {
    ...
    } catch (error) { alert(error.message || "Erreur Match."); }
  },

  async open(person) {
    this.current = person;
    this.render();
    DOM.hide("searchScreen"); DOM.show("matchResult");
    Router.show("match");
  ...
  async friend() {
    ...
    } catch (error) { alert(error.message || "Impossible d'envoyer la demande."); }
  },
