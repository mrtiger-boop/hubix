const Auth = {
  ...
  async init() {
    ...
    if (HubixOnline.ready) {
      try {
        this.user = await HubixOnline.getCurrentUser();
      } catch (error) {
        console.warn("Session Supabase non récupérée :", error);
      }
    }
  ...
  async register() {
    ...
    if (age < 18) return DOM.message("registerMessage", "Hubix est prévu pour les 18+.", "error");
    ...
    try {
      if (HubixOnline.ready) {
        ...
        DOM.message("registerMessage", "Compte en ligne créé.", "success");
      } else {
        if (this.localUsers.some(u => u && u.pseudo && u.pseudo.toLowerCase() === pseudo.toLowerCase())) {
          return DOM.message("registerMessage", "Pseudo déjà utilisé.", "error");
        }
        ...
        HubixLocal.set("hubix_session", this.user);
        DOM.message("registerMessage", "Compte local créé.", "success");
      }
  ...
  async login() {
    ...
    try {
      if (HubixOnline.ready) {
        ...
        HubixLocal.set("hubix_session", this.user);
        DOM.message("loginMessage", "Connexion en ligne réussie.", "success");
      } else {
        const id = identifier.toLowerCase();
        const found = this.localUsers.find(user =>
           user &&
           user.pseudo &&
           user.email &&
           (user.pseudo.toLowerCase() === id || user.email.toLowerCase() === id) &&
        ...
        this.user = found;
        HubixLocal.set("hubix_session", this.user);
        DOM.message("loginMessage", "Connexion locale réussie.", "success");
      }
  ...
  async updateProfile(data, avatarFile) {
    ...
    try {
      if (HubixOnline.ready && this.user.id && !this.user.id.startsWith("local_")) {
        ...
        this.localUsers = this.localUsers.map(user => user.id === this.user.id ? this.user : user);
        HubixLocal.set("hubix_users", this.localUsers);
      }
  ...
  render() {
    ...
    DOM.html("miniAvatar", avatar ? `<img class="avatar-img" src="${avatar}">` : initial);
